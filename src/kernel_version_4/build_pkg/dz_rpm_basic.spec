...
#	alias cdl="cd %{AVM_DIR}/logs"
	...
	echo "   Congratulations !!!"
	echo "   Package is installed successfully in %{CLEAN_DIR}"
	echo "   Use 'make start' command to start AVM"
	echo "   ###################################################################"
	echo ""
...
%preun
	...
	if [ $? -eq 0 ]; then
		echo "   AVM is already started. AISA Target found. "
		echo "   Please stop  AVM  by using 'make stop' and "
		echo "   then uninstall this package. "
		exit 1
	fi
...
#
	...
	#Save the old configuration. Good for recovering if accidentally deleted
	cp -r %{CONFIG_PATH} /tmp/
