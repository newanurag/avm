...
//#define bio->bi_sector DZ_BIO_GET_SECTOR(bio)

RINT dz_process_io(PDM_TARGET target, PBIO bio)
{
	UINT  io_size   = DZ_BIO_GET_SIZE(bio); // Parent or Original bio size
	...
	iorequest->ior_lba 	= dz_convert_sector_to_lba(DZ_BIO_GET_SECTOR(bio));
...
RINT dz_demo_passthrough_writeonly(PDM_TARGET target, struct bio *bio)
{
	...
	//TODO:bio->bi_bdev = aisa_device_object->obj.obj_dm_dev->bdev;
	DZ_BIO_SET_BDEV(bio, aisa_device_object->obj.obj_dm_dev->bdev);
	//TODO:if((bio->bi_rw & WRITE) == WRITE) {
	if (bio_op(bio) == REQ_OP_WRITE) {
		...
		//TODO:dz_process_lab_engine_pipe((bio->bi_sector * SECTOR_SIZE), bio->bi_size);
		dz_process_lab_engine_pipe((DZ_BIO_GET_SECTOR(bio) * SECTOR_SIZE), DZ_BIO_GET_SIZE(bio));
	...
	//TODO:DZ_OS_KERNEL_SUBMIT_BIO(bio->bi_rw,bio);
	return DM_MAPIO_SUBMITTED;
}
...
RINT dz_avm_target_default_map(PDM_TARGET target, struct bio *bio)
{
	...
	//TODO: if((bio->bi_rw & WRITE) == WRITE) {
		...
		LOGTGT("AISA-Info: dz_avm_target_default_map : Failing. bio is a neither READ nor WRITE request.... \n");
		BIODONE(bio, IO_ERROR_EIO);
		return DM_MAPIO_SUBMITTED;
...
RINT  dz_target_create_aisa(struct dm_target *target, UINT argc, CHAR ** argv)
{
	...
	*/
	...
	//TODO:blk_queue_merge_bvec(queue, NULL);
