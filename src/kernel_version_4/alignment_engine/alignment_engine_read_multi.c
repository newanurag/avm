...
static RVOID dz_align_read_parent_biodone(PBIO bio)
{
...
static RVOID dz_align_read_multi_block_head_unaligned_biodone(PBIO bio)
{
...
static RVOID dz_align_read_multi_block_tail_unaligned_biodone(PBIO bio)
{
...
static RVOID dz_align_read_multi_block_both_aligned_biodone(PBIO bio)
{
	//INT 		ret 			= SUCCESS;
	...
	*/
	dz_align_read_parent_biodone(bio);
}
	...
	if (bvec_align == TRUE) {
		...
		for (count = 0; count < total_blocks_needed; count++) {
			...
			if (child_iorequest == prev_child_iorequest) {
				...
				LOGALIGNE("bio->bi_size = %lli, bio->bi_sector = %lli\n", (U64)DZ_BIO_GET_SIZE(bio), (U64)DZ_BIO_GET_SECTOR(bio));
...
bio->bi_vcnt, child_iorequest, child_iorequest->ior_bio,  child_iorequest->ior_type, DZ_BIO_GET_SIZE(child_iorequest->ior_bio), bio_page(child_iorequest->ior_bio));
		...
		for (count = 0; count < total_blocks_needed; count++) {
			...
			//Extra check 
			if (DZ_BIO_GET_SIZE(child_iorequest->ior_bio) > PAGE_SIZE ) {
