...
//TODO:RVOID dz_far_parent_biodone(PBIO bio, INT error)
RVOID dz_far_parent_biodone(PBIO bio)
{
...
RVOID dz_far_multi_block_head_unaligned_biodone(PBIO bio)
{
	...
	*/
	...
	if (atomic_dec_and_test(&piorequest->ior_child_cnt)) {
		// Now complete the Parent / Original bio
		//IO_DONE_STATUS(piorequest, error);
		IOREQUEST_DONE_STATUS(piorequest); //TODO:We need to pass the correct bio status/error
...
RVOID dz_far_multi_block_tail_unaligned_biodone(PBIO bio)
{
	...
	*/
	...
	if (atomic_dec_and_test(&piorequest->ior_child_cnt)) {
		// Now complete the Parent / Original bio
		//IO_DONE_STATUS(piorequest, error);
		IOREQUEST_DONE_STATUS(piorequest);//TODO:We need to pass correct status/error of bio to upper layer
		dz_read_page_free(cbvec.bv_page);
		DZ_OS_KERNEL_BIO_PUT(bio);
		iorequest_put(ciorequest);
...
RVOID dz_far_multi_block_both_aligned_biodone(PBIO bio)
{
	//TODO:INT 		ret 			= SUCCESS;
...
exit_failure:
	//TODO:IO_DONE_ERROR(piorequest, IO_ERROR_ENOMEM);
	IOREQUEST_DONE_ERROR(piorequest);
	iorequest_put(piorequest);
	if (page_head) {
...
//		child_iorequest->target 	= piorequest->target;
	...
	while(aligned_bytes) {
		...
		if (remaining_bv_len) {
				...
				cur_bvec->bv_offset + (cur_bvec->bv_len - remaining_bv_len));
				if (!ret) {
					//TODO:IO_DONE_ERROR(piorequest, ret);
					IOREQUEST_DONE_ERROR(piorequest);
					LOGFECE("Unable to add Page to child bio\n");
					goto exit_page_add;
				}
...
RVOID dz_far_multi_block_tail_unaligned(PIOREQUEST piorequest)
{
	...
	for (count = 0; count < total_blocks_needed; count++) {
		...
		if (child_iorequest == prev_child_iorequest) {
			LOGFEC("First Alloc child_iorequest = %p\n", child_iorequest);
			LOGFEC("Prev Alloc child_iorequest = %p\n", prev_child_iorequest);
			LOGFEC("Both are same\n");
			SL1;
			BUG_ON(1);
		}
...
//		child_iorequest->target 	 = piorequest->target;
	...
	//Set the done function as well. It will be different than the one added above
	//Above child bios have done function dz_far_multi_block_both_aligned_biodone()
	...
	while(aligned_bytes) {
		...
		while(bi_vcnt < bi_vcnt_max) {
			...
			if (bv_len >= data_bytes) {
				...
				if (!ret) {
					IOREQUEST_DONE_ERROR(piorequest);
					LOGFECE("Unable to add Page to child bio instance\n");
					goto exit_page_add;
				}
				...
				if (!ret) {
					IOREQUEST_DONE_ERROR(piorequest);
					LOGFECE("Unable to add Page to child bio extended\n");
					goto exit_page_add;
				}
				data_bytes -= bv_len;
				bi_vcnt++;
...
RINT dz_fec_align_read_for_multi_block(PIOREQUEST piorequest)
{
	...
	if ((sector_bytes  % LBA_BLOCK_SIZE) == 0 ) { 
		...
		if (((sector_bytes + io_size)  % LBA_BLOCK_SIZE) == 0 ) { 
			...
			//LOGFEC("Case1:BIO is Head Aligned and Tail Aligned. io_size = %zu \n", io_size);

			if (ba++ == 0) {
				LOGD("Case1:READ BIO is Head Aligned and Tail Aligned. io_size = %zu bytes\n", io_size);
			}
			LOGFEC("Case1:READ BIO is Head Aligned and Tail Aligned. io_size = %zu bytes\n", io_size);
			...
			LOGFEC("%s", "Case2:Read BIO is Head Aligned and Tail UnAligned\n");
			INC_COUNTER(fec_iocount_reads_multi_unaligned_page);
			INC_COUNTER(fec_iocount_reads_multi_tail_unaligned_page);
		...
		LOGFEC("%s", "Case3:Read BIO is Head UnAligned and Tail Aligned\n");
		INC_COUNTER(fec_iocount_reads_multi_unaligned_page);
		INC_COUNTER(fec_iocount_reads_multi_head_unaligned_page);
		...
		LOGFEC("%s", "Case4:Read BIO is Neither Head Aligned Nor Tail Aligned\n");
		INC_COUNTER(fec_iocount_reads_multi_unaligned_page);
		INC_COUNTER(fec_iocount_reads_multi_both_unaligned_page);
