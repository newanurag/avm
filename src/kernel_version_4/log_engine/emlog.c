...
#define LOGEM(fmt, args...) do { \
    printk (KERN_ERR"[AISA][AVMLOG]:%d:" fmt ,__LINE__, ## args); \
} while (0)
                        ...
                        struct emlog_info **peinfo)
{
    ...
    einfo->size = KB * EMLOG_MAX_SIZE;
	
	LOGEM("Log File Created of size %zu KB \n", (einfo->size)/KB);
...
static int __init dz_emlog_engine_init(void)
{
    int ret_val;
  ...
  emlog_init_okay:
	...
	LOGEM(" ______________________________________________________ \n");
    LOGEM("|                                                      |\n");
    LOGEM("|           ZAIDSTOR EMLog Engine Module Loaded          |\n");
    LOGEM("|______________________________________________________|\n");

    return ret_val;
}

static void __exit dz_emlog_engine_exit(void)
{
	...
	LOGEM(" ______________________________________________________ \n");
    LOGEM("|                                                      |\n");
    LOGEM("|           ZAIDSTOR EMLog Engine Module UnLoaded        |\n");
    LOGEM("|______________________________________________________|\n");
}

module_init(dz_emlog_engine_init);
module_exit(dz_emlog_engine_exit);
