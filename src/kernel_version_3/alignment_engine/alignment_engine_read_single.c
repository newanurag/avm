...
static RVOID dz_align_read_for_single_block_biodone_tail(PBIO bio, INT error)
{
...
static RVOID dz_align_read_for_single_block_biodone_head(PBIO bio, INT error)
{
	PIOREQUEST ciorequest 		= (PIOREQUEST) bio->bi_private;
	PIOREQUEST piorequest 		= ciorequest->ior_parent;
	PVOID   	cpagebuf		= NULL;
...
RVOID static dz_align_fecd_and_bece(PIOREQUEST piorequest)
{
	...
	if (piorequest->ior_parent) {
		...
		bio->bi_vcnt, error, piorequest->ior_type, bio->bi_size, bio, piorequest->ior_bio, piorequest);
		error = dz_align_read_single_block_from_raw_disk_sync_internal(piorequest, sector );
