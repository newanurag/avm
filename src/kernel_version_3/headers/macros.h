...
#define GET_DISK_SECTOR()	\
	...
	#define IO_DONE_ERROR(iorequest, error)			bio_io_error((iorequest)->ior_bio); LOGE("IO_DONE ERROR\n")
	#define IO_DONE_BUSY(iorequest)					bio->bi_status = BLK_STS_DM_REQUEUE;bio_endio((iorequest)->ior_bio); LOGE("IO_DONE BUSY\n")
	#define IO_DONE_STATUS(iorequest, error)	bio_endio((iorequest)->ior_bio, error)
#else									
	#define DZ_BIO_SET_DEV(bio, bdev)				(bio)->bi_bdev = (bdev)	
	#define DZ_BIO_GET_DEV(bio)						(bio)->bi_bdev

	#define DZ_BIO_SET_SIZE(bio, size)				(bio)->bi_size = (size)
	#define DZ_BIO_GET_SIZE(bio)					(bio)->bi_size

	#define DZ_BIO_SET_SECTOR(bio, sector)			(bio)->bi_sector = (sector)
	#define DZ_BIO_GET_SECTOR(bio)					(bio)->bi_sector

	#define DZ_BIO_SET_IDX(bio, idx)				(bio)->bi_idx = (idx)
	#define DZ_BIO_GET_IDX(bio)						(bio)->bi_idx
	#define DZ_BIO_SET_ZIDX(bio)					DZ_BIO_SET_IDX(bio, 0)

	#define DZ_BIO_GET_CNT(bio)						(bio)->bi_cnt

	#define DZ_BIO_SET_WRITE(bio)					(bio)->bi_rw = WRITE
	#define DZ_BIO_SET_READ(bio)					(bio)->bi_rw = READ
	...
	#define DZ_IOREQUEST_GET_SECTOR(piorequest)		(piorequest)->ior_bio->bi_sector

	#define BIO_DONE(bio, error)					bio_endio(bio, error)
	#define IO_DONE(iorequest)						bio_endio((iorequest)->ior_bio, SUCCESS)
	#define READ_IO_DONE(iorequest)					bio_endio((iorequest)->ior_bio, SUCCESS);
	#define READ_IO_DONE_STATUS(iorequest, error)	bio_endio((iorequest)->ior_bio, error);
	#define WRITE_IO_DONE_STATUS(iorequest, error)	bio_endio((iorequest)->ior_bio, error);
	#define WRITE_IO_DONE(iorequest)				bio_endio((iorequest)->ior_bio, SUCCESS);
	#define FAKE_IO_DONE(iorequest)					bio_endio((iorequest)->ior_bio, SUCCESS);
	#define FAKE_BIO_DONE(bio)						bio_endio((bio), SUCCESS);
	#define FAKE_BIODONE(bio)						bio_endio((bio), SUCCESS);
	...
	#define IO_DONE_ERROR(iorequest, error)			bio_endio((iorequest)->ior_bio, error); LOGE("IO_DONE ERROR\n")
	#define IO_DONE_BUSY(iorequest)					bio_endio((iorequest)->ior_bio, DM_ENDIO_REQUEUE); LOGE("IO_DONE BUSY\n")
	#define IO_DONE_STATUS(iorequest, error)	bio_endio((iorequest)->ior_bio, error)

#endif

#define DZ_DDE_IOREQUEST_DONE(iorequest)				IOREQUEST_DONE(iorequest)
#define DZ_DDE_IOREQUEST_DONE_ERROR(iorequest, error)	IOREQUEST_DONE_ERROR((iorequest), (error))
...
#define DZ_ALIGN_IOREQUEST_DONE_ERROR(iorequest, error)	IOREQUEST_DONE_ERROR((iorequest), (error))


#define IOREQUEST_DONE(iorequest)		(iorequest)->ior_donefun(iorequest, SUCCESS)
#define IOREQUEST_DONE_ERROR(iorequest, error) (iorequest)->ior_donefun(iorequest, (error))
