...
static RVOID dz_align_write_partial_block_parent_biodone(PBIO bio, INT error)
{
...
static RVOID dz_align_write_partial_block_read_page_sync_biodone(PBIO bio, INT error)
{
...
static RVOID dz_align_write_partial_block_double_page_async_biodone_head(PBIO bio, INT error)
{
	PIOREQUEST	ciorequest 	= (PIOREQUEST)(bio->bi_private);
	PIOREQUEST	piorequest 	= ciorequest->ior_parent;
	...
	dz_align_write_partial_block_parent_biodone(bio, error);
...
static RVOID dz_align_write_partial_block_double_page_async_biodone_tail(PBIO bio, INT error)
{
	PIOREQUEST	ciorequest 	= (PIOREQUEST)(bio->bi_private);
	PIOREQUEST	piorequest 	= ciorequest->ior_parent;
	PBIO		parent_bio	= piorequest->ior_bio;
...
static PPAGE dz_align_write_partial_block_read_page_sync(PIOREQUEST iorequest, PBIO parent_bio, SECTOR sector)
{
	...
	bio->bi_end_io 	= dz_align_write_partial_block_read_page_sync_biodone;
	DZ_BIO_SET_ZIDX(bio);
	bio->bi_rw 		= READ;
	bio->bi_next 	= NULL;
...
RVOID dz_align_write_for_partial_block(PIOREQUEST parent_iorequest)
{
	...
	SECTOR	sector 				= parent_bio->bi_sector;
	LBA		lba					= parent_iorequest->ior_lba;
	UINT	sector_pos_in_lba	= 0;
	INT		size				= parent_bio->bi_size;
...
do_rmw: //Do Read Modify Write i.e. Read a Page and merge the desired changes
		...
		bio_for_each_segment(pbvec, (parent_bio), i) {
			pagebuf = kmap(pbvec->bv_page);
			pagebuf += pbvec->bv_offset;
